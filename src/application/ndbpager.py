"""
Original file was taken from here: https://github.com/vmihailenco/ndbpager/blob/master/ndbpager/base.py
"""
import hashlib
...
class BasePager(object):
    def __init__(self, **kwargs):
        try:
            self._page = int(kwargs.pop('page', 1))
        except ValueError:
            self._page = 1
        if self._page < 1:
            self._page = 1
...
class Pager(BasePager):
    ...
    def paginate(self, page_size=20, **q_options):
        if self._page > 1:
            cursor, more = self._get_from_cache(self._page - 1)
            if not cursor:
                self._page, cursor, _ = self._get_max_avail_page(page_size)
        else:
            cursor = None
        ...
        if cursor:
            self._add_to_cache(self._page, cursor, more)
        self.has_next = more
